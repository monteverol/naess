  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        const foundPrincipal = principal.find((item) => item.slug.toString() === slug);
  ...
  return(
    <>
      <section className={`bg-white mt-20 px-8 md:px-20 lg:px-40 py-8 flex flex-col gap-8 lg:flex-row ${principalData.vessels ? "justify-between items-start" : "justify-center items-center"}`}>
        <article className="w-full lg:min-w-4xl lg:max-w-4xl flex flex-col items-center lg:items-start">
